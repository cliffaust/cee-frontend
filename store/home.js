...
export const actions = {
  async addHome({ commit }, payload) {
    ...
    let home = []
    if (payload.token) {
      home = await axios.get(`${process.env.baseUrl}/homes/${payload.slug}/`, {
        headers: {
    ...
    const homeImages = await axios.get(
      `${process.env.baseUrl}/homes/${payload.slug}/home-images/`
    )
    ...
    data.push({
      home_images: [
        { home_image: home.data.cover_image },
        ...homeImages.data.results,
      ],
    })
